...
class AdamOptim(Optim):
    def __init__(self, net: nn_impl.nnImpl, lr : float):
        ...
        self.V_w = [np.ones(l.w_.shape) for l in self.net.layers]
        self.G_w = self.V_w.copy()

        self.V_b = [np.ones(l.b_.shape) for l in self.net.layers]
    ...
    def descend_w_bias(self):
        for i, layer in enumerate(self.net.layers):
            ...
            layer.w_.val -= self.lr / np.sqrt(self.G_w[i] + self.eps) * self.V_w[i]
            layer.b_.val -= self.lr / np.sqrt(self.G_b[i] + self.eps) * self.V_b[i]
